...
export class ScratchRuntimeManager implements InternalMouseListener {
    ...
    constructor(private interpreter: Interpreter, private world: IWorld) {
    ...
    private wheelListener = (e: WheelEvent) => {
        const list = this.lists.mouseWheelMoved;
     ...
     */
    broadcast(message: string | StringClass) {
        const list = this.lists.iReceive;
    ...
    /** Notify every receiver that overrides whenBackdropSwitches(String). */
    backdropSwitch(name: string) {
        const list = this.lists.backdropSwitches;
        if (list.length === 0) return;
    ...
    onKeyPressed(browserKey: string) {
        const list = this.lists.keyPressed;
        if (list.length === 0) return;
    ...
    onKeyReleased(browserKey: string) {
        const list = this.lists.keyReleased;
        if (list.length === 0) return;
        const code = this.keyCodeFor(browserKey);
        if (!code) return;
    ...
    onMouseEvent(kind: MouseEventKind, x: number, y: number): void {
        ...
        const clicked = this.lists.clicked;
        const mouseClicked = this.lists.mouseClicked;
    ...
    private fireMouseMoved(screenX: number, screenY: number) {
        const list = this.lists.mouseMoved;
        if (list.length === 0) return;
