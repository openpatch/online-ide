...
import { activeScratchStage, isScratchStageActive } from "./ScratchStages";
...
export class ScratchRuntimeManager implements InternalMouseListener {
     ...
     * react to someone typing their name — the same guard upstream's keyEvent
     * puts in front of whenKeyPressed/whenKeyReleased.
     */
    private stageIsAsking(): boolean {
        return activeScratchStage<{ _isAsking?: () => boolean }>()?._isAsking?.() === true;
    }

    onKeyPressed(browserKey: string) {
        if (this.stageIsAsking()) return;
